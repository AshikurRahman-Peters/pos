...
class Products extends CI_Controller {
    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Name, ' - ', br.brand_name) as display_text,
               round( p.Product_SellingPrice / 24,3) as Product_SellingpicPrice,
               round( p.Product_Purchase_Rate / 24,3) as Product_Purchase_pic_Rate,
                0 as discount,
    ...
    public function getCurrentStock(){
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    concat(p.Product_Name, ' - ', b.brand_name) as Product_Name,
                    p.Product_Code,
                    ...
                    (select sum(PurchaseDetails_Discount) from tbl_purchasedetails 
                        where Product_IDNo=p.Product_SlNo
                    ) as free_product_qty
        ...
        $res['totalValue'] = array_sum(
         ...
         $res['free_quantity'] = array_sum(
            array_map(function($product){
                return $product->free_product_qty;
            }, $stock));
    ...
    public function getTotalStock(){
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pd.PurchaseDetails_Discount), 0) 
                        from tbl_purchasedetails pd 
                        where pd.Product_IDNo = p.Product_SlNo
                        and pd.PurchaseDetails_branchID = '$branchId'
                        and pd.Status = 'a') as purchased_free_product,
